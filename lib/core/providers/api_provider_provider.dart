...
class ApiProviderListNotifier extends AsyncNotifier<List<ApiProviderData>> {
  ...
  Future<List<ApiProviderData>> build() async {
    ...
    try {
      ...
      // 表可能不存在或 schema 不兼容（TEXT datetime vs INTEGER timestamp），重建
      try {
        await db.customStatement('DROP TABLE IF EXISTS api_providers');
  ...
  String _generateId() {
    ...
    return now.toRadixString(36) +
        (now % 10000).toRadixString(36).padLeft(3, '0');
  }
}
