...
///
/// 使用 StateNotifierProvider.family（非 autoDispose）：
/// - 每个 conversationId 拥有独立的 ChatController 实例
/// - 实例在监听者移除后不会被自动销毁
/// - 这确保用户切换对话时，后台正在进行的流式生成能继续完成并持久化结果
/// - 每个实例拥有独立的 CancelToken，stop() 仅影响对应对话
    ...
    try {
      ...
      if (settings.memoryInject) {
        ...
        final relevant = await memoryEngine.retrieveRelevantMemories(
          ...
          maxMemories: settings.limitInject ? settings.memoryMaxInject : 9999,
        );
      ...
      // 6. 调用 LLM
      // 注意：由于 chatControllerProvider 非 autoDispose，即使用户切换到其他对话，
      // 本 ChatController 实例仍然存活，onDone/onError 回调会正常执行并持久化结果。
