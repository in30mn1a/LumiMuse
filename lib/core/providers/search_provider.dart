...
/// 对 SQL LIKE 关键字做转义：将反斜杠、百分号、下划线转为字面量，
/// 配合 `ESCAPE '\'` 子句防止用户输入中的通配符被 SQLite 解释为特殊匹配符。
///
/// 例如用户搜索 "100%电量" 时，若不转义 `%` 会命中所有含 "100" 的记录；
/// 转义后 `%` 退化为普通字符，仅匹配含 "100%电量" 连续片段的记录。
String _escapeLikeWildcards(String s) {
  return s
      .replaceAll('\\', '\\\\')
      .replaceAll('%', '\\%')
      .replaceAll('_', '\\_');
