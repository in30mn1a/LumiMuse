...
class ImagePromptService {
  ...
  Future<({String positive, String negative})> generateImagePrompt(
    ...
    return _parsePromptResponse(result);
  }
  ...
  static ({String positive, String negative}) _parsePromptResponse(String text) {
    ...
    if (positiveIndex < negativeIndex) {
      final positive =
          text.substring(positiveIndex + 'POSITIVE:'.length, negativeIndex);
      final negative = text.substring(negativeIndex + 'NEGATIVE:'.length);
      return (positive: positive.trim(), negative: negative.trim());
    }

    final negative =
        text.substring(negativeIndex + 'NEGATIVE:'.length, positiveIndex);
    final positive = text.substring(positiveIndex + 'POSITIVE:'.length);
    return (positive: positive.trim(), negative: negative.trim());
  }
