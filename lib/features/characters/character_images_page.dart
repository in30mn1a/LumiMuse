...
class _CharacterImagesPageState extends ConsumerState<CharacterImagesPage> {
  ...
  void _previewItem(CharacterImageItem item) {
    final group = _items
        .where((it) =>
            it.messageId == item.messageId && it.imageId == item.imageId)
        .toList();
    // 列表里同 image 内多版本是「最新在前」，预览器期望按版本时间升序，
    // 这里反向一次得到「最早在前」，与消息气泡 lightbox 保持一致
    final sortedPaths = group.reversed.map((e) => e.localPath).toList();
  ...
  List<Widget> _buildHeaderActions() {
    final canDelete =
        _selectionMode && _selectedKeys.isNotEmpty && !_deleting;
    return <Widget>[
      AppPageActionChip(
