...
class ChatActions {
  ...
  Future<void> sendFromInput(
    ...
    if (attachments != null && attachments.isNotEmpty) {
      await controller.sendMessageWithAttachments(text, attachments);
    } else {
  ...
  Future<void> generateImageForMessage(String messageId, {String? existingPrompt, String? replaceImageId}) async {
    ...
    // 对照主项目：replaceImageId 有值时复用该 ID（原位替换），否则生成新 ID
    final placeholderId = replaceImageId ??
        (DateTime.now().millisecondsSinceEpoch.toRadixString(36) +
            (1000 + (messageId.hashCode & 0xFFF)).toRadixString(36));
  ...
  Future<void> generateImage(String userHint) async {
    ...
    try {
      ...
      final placeholderMeta = MessageMetadata(
        generatedImages: [
          GeneratedImage(id: 'pending_image', url: '', path: '', prompt: p.positive, status: 'pending'),
        ],
      );
      ...
      final doneMeta = MessageMetadata(
        generatedImages: [
          GeneratedImage(id: '', url: '', path: imagePath, prompt: p.positive, status: 'ready'),
        ],
      );
      await (db.update(db.messages)..where((t) => t.id.equals(placeholderId!))).write(
