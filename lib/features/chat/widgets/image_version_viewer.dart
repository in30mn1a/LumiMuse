...
class _ImageVersionViewerState extends ConsumerState<ImageVersionViewer> {
  ...
  /// InteractiveViewer 的变换控制器 — 用于实时监听缩放比例
  ///
  /// 替代不可靠的 onInteractionUpdate（缩放结束后无法回到 _isScaled = false），
  /// 通过监听 TransformationController 的矩阵变化推导真实缩放，使
  /// "缩放回 1x 自动恢复水平滑动切换" 这一行为可靠工作。
  ...
  void _onTransformChanged() {
    ...
    // 留 0.01 容错，避免浮点抖动反复触发
    final scaled = scale > 1.01;
    if (scaled != _isScaled && mounted) {
      setState(() => _isScaled = scaled);
    }
  }
  ...
  void _handleDoubleTap() {
    ...
    if (currentScale > 1.01) {
      ...
      if (renderBox != null) {
        ...
        const double s = 2.5;
        final double tx = (size.width - size.width * s) / 2;
        final double ty = (size.height - size.height * s) / 2;
        _transformationController.value = Matrix4.identity()
  ...
  Widget _buildImageView({
    ...
    return GestureDetector(
      ...
      child: InteractiveViewer(
        ...
        minScale: 1.0,
        maxScale: 4.0,
        // _isScaled 由 _onTransformChanged 通过监听 TransformationController 推导，
        // 不再依赖 onInteractionUpdate / onInteractionEnd（缩放结束后无法可靠还原）。
        child: Image.file(
          file,
          key: key,
          fit: BoxFit.contain,
