...
class _MessageBubbleState extends ConsumerState<MessageBubble> {
  ...
  MarkdownStyleSheet _getMarkdownStyle(BuildContext context) {
    ...
    if (_markdownStyleSheet == null || _lastBrightness != brightness) {
      _lastBrightness = brightness;
      _markdownStyleSheet = _buildMarkdownStyle();
    }
  ...
  bool _metaIsSummary(MessageMetadata meta) => meta.isSummary;

  _VersionInfo? _versionInfoFromMeta(MessageMetadata meta) {
  ...
  Widget build(BuildContext context) {
    // 一次性解析 metadata，避免在多个子方法中重复反序列化（P1 优化）
    final meta = _parseMeta();
    if (widget.message.role == 'system' || _metaIsSummary(meta)) {
      return _SummaryCard(
        ...
        children: [
          ...
          Flexible(
            child: Column(
              ...
              children: [
                _buildBubble(isUser, meta),
                _buildGeneratedImages(meta),
              ],
            ),
          ),
        ],
  ...
  Widget _buildBubble(bool isUser, MessageMetadata meta) {
                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (isUser) _buildUserAttachments(isDark, meta),
                      if (widget.message.content.isNotEmpty) ...[
                        const SizedBox(height: 6),
                        _buildContent(isUser),
                      ],
                      _buildVersionNav(isUser, meta),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildUserAttachments(bool isDark, MessageMetadata meta) {
  ...
  Widget _buildVersionNav(bool isUser, MessageMetadata meta) {
    final info = _versionInfoFromMeta(meta);
  ...
  Widget _buildGeneratedImages(MessageMetadata meta) {
  ...
  ///
  /// 用独立的 [_EditImagePromptDialog] StatefulWidget 承载 controller，
  /// 在其 dispose 中释放，避免每次 showDialog 都泄漏 TextEditingController。
  void _showEditPromptDialog(String imageId, String currentPrompt) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    showDialog(
      context: context,
      builder: (ctx) => _EditImagePromptDialog(
        initialPrompt: currentPrompt,
        isDark: isDark,
        onSave: (newPrompt) {
          widget.onEditImagePrompt?.call(imageId, newPrompt);
        },
        onSaveAndRegenerate: (newPrompt) {
          widget.onEditImagePrompt?.call(imageId, newPrompt);
          // 保存并重新生成 — 在原位替换（传入 replaceImageId）
          widget.onGenerateImage
              ?.call(prompt: newPrompt, replaceImageId: imageId);
        },
      ),
    );
  }
  ...
  Widget _buildImageEntry({
    ...
    return ImageAttachment(
      ...
      onEditPrompt: widget.onEditImagePrompt != null
          ...
          : null,
      onDelete: widget.onDeleteGeneratedImageVersion != null
          ? () => widget.onDeleteGeneratedImageVersion!(imageId, path)
          : (widget.onDeleteGeneratedImage != null
              ...
              child: InkWell(
                ...
                child: Padding(
                  ...
                  child: Icon(
                    ...
                    color: _isHovering ? hoverColor : base,
                  ),
                ),
              ),
            ),
...
class _VersionButtonState extends State<_VersionButton> {
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      onExit: (_) => setState(() => _isHovering = false),
      child: AnimatedScale(
        scale: _isHovering && isClickable ? 1.1 : 1.0,
        duration: const Duration(milliseconds: 150),
        curve: Curves.easeOutBack,
        child: ConstrainedBox(
...
/// 编辑图片提示词对话框
///
/// 抽成独立 StatefulWidget，使 [TextEditingController] 在 dispose 中释放，
/// 避免 [_MessageBubbleState._showEditPromptDialog] 中 controller 泄漏。
class _EditImagePromptDialog extends StatefulWidget {
  final String initialPrompt;
  final bool isDark;
  final ValueChanged<String> onSave;
  final ValueChanged<String> onSaveAndRegenerate;

  const _EditImagePromptDialog({
  ...
  @override
  State<_EditImagePromptDialog> createState() => _EditImagePromptDialogState();
}

class _EditImagePromptDialogState extends State<_EditImagePromptDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () {
            final newPrompt = _controller.text;
            Navigator.pop(context);
            widget.onSaveAndRegenerate(newPrompt);
          },
