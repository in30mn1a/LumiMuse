...
class _ResetExtractionDialogState
    ...
    return Wrap(
      ...
      children: [
        ...
        _SmallBtn(
          ...
          kind: _SmallBtnKind.secondary,
          isDark: isDark,
          onTap: () => _execute(action: 'reset', allTargets: true),
        ),
        // 标记已提取（选中）
        _SmallBtn(
          ...
          onTap: _selected.isEmpty
              ? null
              : () => _execute(action: 'mark', allTargets: false),
        ),
        // 重置选中
        _SmallBtn(
          ...
          onTap: _selected.isEmpty
              ? null
              : () => _execute(action: 'reset', allTargets: false),
        ),
      ],
    );
  }
  ...
  Future<void> _execute({
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.of(context).pop();
      final actionText = action == 'mark' ? '标记已提取' : '重置';
      widget.onToast(
...
class _MessageRow extends StatelessWidget {
  ...
  bool get _extracted {
    // metadata 是 JSON 字符串，简单按子串匹配（精确反序列化交给 conversationActions）
    return message.metadata.contains('"memory_extracted":true');
  }
