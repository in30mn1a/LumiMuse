      ...
      child: Column(
        ...
        children: [
          ...
          _editField(
            ...
            placeholder: 'OpenAI',
            onChanged: (v) =>
                setState(() => _editingProvider!['name'] = v),
          ),
          const SizedBox(height: 12),
          _editField(
            isDark: isDark,
            ...
            onChanged: (v) =>
                setState(() => _editingProvider!['api_base'] = v),
          ),
          const SizedBox(height: 12),
          _editField(
            isDark: isDark,
            ...
            obscure: true,
            onChanged: (v) =>
                setState(() => _editingProvider!['api_key'] = v),
          ),
          const SizedBox(height: 12),
          _editField(
            isDark: isDark,
            ...
            onChanged: (v) =>
                setState(() => _editingProvider!['model'] = v),
          ),
          const SizedBox(height: 12),
          Row(
            children: [
    ...
    return LumiSectionPanel(
      ...
      child: LayoutBuilder(
        builder: (ctx, constraints) {
          ...
          final fontBlock = SettingsLabeledFieldFramed(
            label: I18n.t('settings.font', lang: lang),
            child: SettingsFontPickerRow(
              current: settings.fontStyle,
              onPick: (f) => updateSetting('font_style', f),
              lang: lang,
            ),
          );
          if (wide) {
...
class _MaintenanceSectionState extends ConsumerState<MaintenanceSection> {
  ...
  Future<void> _maintCleanup() async {
    ...
    try {
      final db = ref.read(databaseProvider);
      final service = MaintenanceService(db);
      final count = await service.cleanOrphans();
      if (mounted) {
        setState(() {
          _maintCleanedCount = count;
          _maintStatus = _MaintStatus.done;
          if (count > 0) _statsRefreshSeq++;
        });
