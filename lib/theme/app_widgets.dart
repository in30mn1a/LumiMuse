          ...
          child: AnimatedContainer(
            ...
            child: Row(
              ...
              children: [
                ...
                if (widget.label != null) Text(widget.label!, style: textStyle),
              ],
            ),
          ),
        ),
...
class _LumiChipState extends State<LumiChip> {
  ...
  Widget _buildIconChip(bool isDark, bool disabled) {
    ...
    if (widget.tooltip != null) {
      return Tooltip(message: widget.tooltip!, child: childWidget);
    }
    return childWidget;
  }
...
class LumiNetworkImage extends StatelessWidget {
  ...
  Widget _buildNetworkImage(Widget defaultPlaceholder, Widget defaultError) {
    // Image.network 是构造函数，不会同步抛异常；错误统一由 errorBuilder 处理。
  ...
  Widget _buildLocalImage(Widget defaultError) {
    ...
    if (isAssetLike) {
      // Image.asset 是构造函数，不会同步抛异常；错误统一由 errorBuilder 处理。
      return Image.asset(
        url,
        width: width,
        height: height,
