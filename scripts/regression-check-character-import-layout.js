...
assert(editorPage.includes('file: File'), '待导入备份保留原始文件对象，避免前端提前整包读取');
...
assert(editorPage.includes('target_character_id'), '导入当前角色备份会指定当前角色作为目标');
assert(editorPage.includes('body: pendingImport.file'), '确认导入时直接上传原始文件');
assert(editorPage.includes('characterDraft'), '后端返回角色资料草稿后覆盖当前表单');
assert(!editorPage.includes('JSON.parse(await file.text())'), '角色编辑页不再用 file.text() 读取整份备份');
...
assert(importRoute.includes('target_character_id'), '后端导入支持指定当前角色作为导入目标');
assert(importRoute.includes('characterDraft'), '后端导入可返回角色资料草稿');
assert(!importRoute.includes('MAX_IMPORT_BYTES'), '后端导入不再使用 10MB 硬上限');
assert(!importRoute.includes('Content-Length'), '后端导入不再要求 Content-Length');
