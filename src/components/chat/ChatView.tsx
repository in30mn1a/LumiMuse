...
export default function ChatView({ character, conversationId, targetMessageId, onOpenSidebar, onOpenSearch }: Props) {
  ...
  // 自动生图：持有最新的 handleGenerateImage 引用（它在组件后段才定义），
  // 供"流完成刷新 effect"调用，避免前向引用与闭包过期。
  const handleGenerateImageRef = useRef<((messageId: string, existingPrompt?: string, replaceImageId?: string, conversationIdOverride?: string) => void) | null>(null);
  // 已自动触发过生图的 assistant 消息 id，避免重复出图
  ...
  // 同时清掉"提取状态自动复位"的 setTimeout,避免它在组件卸载后触发 setMemoryExtractStatus。
  useEffect(() => () => {
  ...
  const memoriesTokens = useMemo(() => {
    ...
    return Math.min(estimateTokens(memText), memoryPackageBudget);
  ...
  useEffect(() => {
    ...
    fetchMessagesPage(loadingConvId, { limit: PAGE_SIZE, all: needsTarget, signal: ctl.signal })
      ...
      .catch(() => {});
    return () => ctl.abort();
  }, [activeConvId, applyMessagesResponse, targetMessageId]);
  ...
  // 自动生图：根据最新设置 + 刚完成的消息列表，决定是否自动出图。
  // 用 freshMessages（服务端权威数据，含 metadata.inlineImagePrompt）直接触发，规避 ref 同步竞态。
  const maybeAutoGenerateImageFromMessages = useCallback(async (cid: string, freshMessages: Message[]) => {
    try {
      ...
      // 该消息已触发过或已有生成图片，跳过（避免重复出图）
      if (autoImagedMsgIdsRef.current.has(lastAssistant.id)) return;
      const existingImgs = (lastAssistant.metadata as Record<string, unknown> | undefined)?.generatedImages as unknown[] | undefined;
      if (existingImgs && existingImgs.length > 0) return;
      ...
      if (imgCfg.inline_prompt && typeof inlinePrompt === 'string' && inlinePrompt.trim()) {
        ...
        triggerPrompt = undefined; // 走慢速接口现生成
      } else {
        return;
      }

      autoImagedMsgIdsRef.current.add(lastAssistant.id);
      handleGenerateImageRef.current?.(lastAssistant.id, triggerPrompt, undefined, cid);
    } catch {
      // 自动生图失败不影响主流程
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // maybeAutoGenerateImageFromMessages 用 ref 读取依赖，无需进依赖数组
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeStreams, activeConvId, applyMessagesResponse, messages.length]);
  ...
  const refreshMessages = useCallback(async () => {
    ...
    const response = await fetchMessagesPage(convId, { limit: Math.max(PAGE_SIZE, messagesRef.current.length) });
  ...
  const refreshMessagesForConversation = useCallback(async (conversationIdToRefresh: string) => {
    ...
    const response = await fetchMessagesPage(conversationIdToRefresh, { limit: Math.max(PAGE_SIZE, loadedLength) });
  ...
  }, [t, updateMessagesForConversation]);
  ...
  const handleGenerateImage = useCallback(async (messageId: string, existingPrompt?: string, replaceImageId?: string, conversationIdOverride?: string) => {
    ...
    // 优先级：显式传入的 existingPrompt > 该消息内联提示词（[IMG] 块，聊天时已生成）> 调用慢速接口现生成
  ...
  }, [showToast, t, updateMessagesForConversation]);

  // 保持 ref 指向最新的 handleGenerateImage，供"流完成刷新 effect"调用
  useEffect(() => {
  ...
  const handleResetExtraction = async (messageIds?: string[], action: 'reset' | 'mark' = 'reset') => {
    ...
    try {
      ...
      serverUnextractedCountRef.current = nextUnextractedCount;
      setServerUnextractedCount(nextUnextractedCount);
