      ...
      <div className="surface-panel w-full max-w-md p-5">
        ...
        <div className="mt-3 flex items-center justify-between text-sm">
          <span className="text-text-muted">
            {selectedIds.size > 0 ? `已选 ${selectedIds.size} 条` : '点击消息多选'}
          </span>
        ...
        {allMessages === null ? (
          ...
          <div className="mt-2 max-h-64 overflow-y-auto rounded-xl border border-border-light">
            ...
            {hasMore && (
              ...
              >
                加载更多（还有 {userMsgs.length - visibleCount} 条）
              </button>
            )}
          </div>
        )}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          >
            {conversation?.ignore_memory ? '✓ 已忽略' : '忽略提取'}
          </button>
          {/* 手动提取按钮 */}
          <button
