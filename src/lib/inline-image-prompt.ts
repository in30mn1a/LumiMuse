...
export function buildInlinePromptInstruction(imageTags?: string, userImageTags?: string): string {
  ...
  const userPart = userImageTags && userImageTags.trim()
    ...
    : '';
  const soloPart = '；如果不是角色与用户的亲密互动，不得包含任何用户外貌标签，必须生成只包含角色的单人场景';
  return `（系统附加要求，务必执行，不算跳出角色）在以上角色对话正文之后，另起一行追加一段英文 danbooru 文生图标签串，用 [IMG] 和 [/IMG] 包裹，描述本条回复对应的画面。要求：不少于 35 个标签，英文逗号分隔；依次覆盖【画面构成】(分级 sfw/nsfw、人数如 1girl、画幅、视角、镜头)、【外貌】(发型发色瞳色体型${tagPart}${userPart}${soloPart})、【服饰】(逐件列出)、【动作】(贴合本条剧情)、【表情】(视线情绪面部细节)、【场景】(地点环境时间光影)；默认第三人称，可以在亲密互动时适当使用POV视角；[IMG] 块只出现一次放最后；正文不要提及标签；无论对话长短都不要省略。`;
}
