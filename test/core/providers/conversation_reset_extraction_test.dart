  ...
  group('Property 14: resetExtraction round-trip', () {
    Glados<List<bool>>(any.assistantPreMarkedFlags).test(
      ...
      (preMarked) async {
        ...
        final result =
            await actions.resetExtraction('conv-1', action: 'reset');
    ...
    Glados<List<bool>>(any.assistantPreMarkedFlags).test(
      ...
      (preMarked) async {
        ...
        final actions = ConversationActions(db);
        final result = await actions.resetExtraction('conv-1', action: 'mark');

        // affectedCount == 预先未标记的条数
        final expectedAffected = preMarked.where((b) => !b).length;
        expect(result.affectedCount, expectedAffected);
        expect(result.action, 'mark');
    ...
    Glados<List<bool>>(any.assistantPreMarkedFlags).test(
      ...
      (preMarked) async {
        ...
        final actions = ConversationActions(db);
        await actions.resetExtraction('conv-1', action: 'mark');
        final second =
            await actions.resetExtraction('conv-1', action: 'mark');
        expect(second.affectedCount, 0);
      },
    ...
    test('空对话（无 assistant 消息）reset 与 mark 均返回 0', () async {
      ...
      final actions = ConversationActions(db);
      final r1 = await actions.resetExtraction('conv-empty', action: 'reset');
      expect(r1.affectedCount, 0);
      final r2 = await actions.resetExtraction('conv-empty', action: 'mark');
      expect(r2.affectedCount, 0);
    });

    test('非法 action 抛 ArgumentError(invalidAction)', () async {
      final db = _createTestDb();
      addTearDown(() => db.close());
      await _seedConversation(
        db,
        characterId: 'char-1',
        conversationId: 'conv-1',
        preMarked: const <bool>[false, true],
      );

      final actions = ConversationActions(db);
      expect(
        () => actions.resetExtraction('conv-1', action: 'unknown'),
        throwsA(isA<ArgumentError>()
            .having((e) => e.message, 'message', 'invalidAction')),
      );
    });
  });
}
