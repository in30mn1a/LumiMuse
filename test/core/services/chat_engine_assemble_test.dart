  ...
  group('Property 8: summary 截断保持后续消息（4.1, 4.2, 4.3）', () {
    ...
    Glados<List<Message>>(any.messageListWithSummaryFlags).test(
      ...
      (messages) {
        ...
        expect(effective.length, expectedLen,
            reason: '截断结果长度公式不匹配');

        // 同时校验：命中时第 0 项就是 summary 自身（包含 summary）。
        if (idx >= 0) {
          expect(_isSummaryMeta(effective.first.metadata), isTrue,
  ...
  group('Property 9: 相邻同 role 合并（4.4, 4.5）', () {
    Glados<List<ChatMessage>>(any.chatMessageListForMerge).test(
      ...
      (chatMessages) {
        ...
        for (int i = 0; i + 1 < merged.length; i++) {
          ...
          expect(same && nonSystem, isFalse,
              reason: '不应存在相邻同 role 的非 system 消息');
        }
      },
    );
    ...
    Glados<List<ChatMessage>>(any.chatMessageListForMerge).test(
      ...
      (chatMessages) {
        ...
        // 段数必须与 merged 长度一致。
        expect(merged.length, segments.length,
            reason: '合并段数应等于按规则分段的段数');

        for (int i = 0; i < merged.length; i++) {
          final seg = segments[i];
          // role 与该段第一条一致（合并保留前者 role）。
          expect(merged[i].role, seg.first.role,
              reason: '第 $i 段合并后 role 应与原段首条一致');
          // content 等于该段内所有 content 按 `\n\n` 拼接。
          final expectedContent =
              seg.map((m) => m.content as String).join('\n\n');
          expect(merged[i].content, expectedContent,
...
Message _makeMsg(String id, {String metadata = '{}'}) {
  return Message(
    id: id,
