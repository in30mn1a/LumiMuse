...
Future<void> _seedDatabase(AppDatabase db, {List<String>? imagePaths}) async {
  final paths =
      imagePaths ?? ['/fake/v-0.png', '/fake/v-1.png', '/fake/v-2.png'];

  /// 三版本展开后会得到 3 个网格条目（最新版本即数组末尾排在前）。
  await db
      .into(db.characters)
      .insert(
        CharactersCompanion.insert(id: _charId, name: const Value('图片管理页测试角色')),
      );
  await db
              ...
              'generatedImages': [
                {
                  ...
                  'activeVersion': 2,
                  'versions': [
                    {'id': 'v-0', 'url': paths[0], 'prompt': 'p0'},
                    {'id': 'v-1', 'url': paths[1], 'prompt': 'p1'},
                    {'id': 'v-2', 'url': paths[2], 'prompt': 'p2'},
                  ],
                },
              ],
            }),
...
Widget _buildPageApp(AppDatabase db) {
  return ProviderScope(
    ...
    child: const MaterialApp(home: CharacterImagesPage(characterId: _charId)),
  );
}
  ...
  final images = tester
      .widgetList<Image>(
        find.byWidgetPredicate(
          (widget) => widget is Image && widget.key is ValueKey<int>,
        ),
      )
      .toList();
  expect(images, hasLength(1));
  return images.single.image as FileImage;
}

void main() {
  group('Widget · CharacterImagesPage 多选删除流程', () {
    testWidgets('渲染网格 → 长按进入多选 → 切换选中 → 批量删除 → SnackBar 与列表刷新', (tester) async {
      ...
      expect(_countTiles(), 1, reason: '删除两个版本后，剩余条目应为 1');
      // 删除完成后多选态应自动退出
      expect(
        find.textContaining('已选 '),
        findsNothing,
    ...
    testWidgets('点击中间版本预览后，左箭头回到列表左侧版本，右箭头回到列表右侧版本', (tester) async {
      final db = _createTestDb();
      addTearDown(() => db.close());

      final dir = Directory.systemTemp.createTempSync('lumimuse-images-page-');
      addTearDown(() {
        if (dir.existsSync()) {
          dir.deleteSync(recursive: true);
        }
      });
      final v0 = writeTestPng(dir, 'v0.png');
      final v1 = writeTestPng(dir, 'v1.png');
      final v2 = writeTestPng(dir, 'v2.png');
      ...
      await tester.pumpAndSettle();
      expect(_currentPreviewImage(tester).file.path, v1.path);

      await tester.tap(find.byIcon(Icons.chevron_left));
      await tester.pumpAndSettle();
      expect(
        _currentPreviewImage(tester).file.path,
        v2.path,
        reason: '左箭头应切到图片管理网格中当前项左侧的版本',
      );
