  ...
  await withTempWorkspace(async () => {
    try {
      ...
      db.prepare(`
        ...
        VALUES ('conv-a', 'char-a', '画像采样', '2026-06-05T00:00:00.000Z', '2026-06-05T00:03:00.000Z')
      `).run();
      const insertMessage = db.prepare(`
        INSERT INTO messages (id, conversation_id, role, content, created_at, seq)
        VALUES (?, 'conv-a', ?, ?, ?, ?)
      `);
      insertMessage.run('msg-user-1', 'user', '主人说正在验证画像初始化采样。', '2026-06-05T00:00:00.000Z', 1);
      insertMessage.run('msg-assistant-1', 'assistant', '艾莉丝会记录这次采样验证。', '2026-06-05T00:01:00.000Z', 2);
      insertMessage.run('msg-user-2', 'user', '主人希望采样仍保留助手回复。', '2026-06-05T00:02:00.000Z', 3);
      insertMessage.run('msg-assistant-2', 'assistant', '艾莉丝会保留下一条回复配对。', '2026-06-05T00:03:00.000Z', 4);
      db.prepare(`
      ...
      assert.match(capturedPrompt, /主人说正在验证画像初始化采样/);
      assert.match(capturedPrompt, /艾莉丝会保留下一条回复配对/);
...
test('memory profile init_from_memories 只读取采样用户消息的下一条 assistant', async () => {
  const sampledUsers = Array.from({ length: 10 }, (_, index) => {
    const number = index + 1;
    return {
      id: `user-${number}`,
      content: `采样用户消息 ${number}`,
      created_at: `2026-06-05T00:${String(index * 2).padStart(2, '0')}:00.000Z`,
      seq: number * 2 - 1,
    };
  });
  const assistantRows = sampledUsers.map((user, index) => ({
    user_id: user.id,
    content: `采样助手回复 ${index + 1}`,
    created_at: `2026-06-05T00:${String(index * 2 + 1).padStart(2, '0')}:00.000Z`,
    seq: user.seq + 1,
  }));
  const unexpectedTailRows = Array.from({ length: 200 }, (_, index) => ({
    role: index % 2 === 0 ? 'user' : 'assistant',
    content: `未采样尾部消息 ${index + 1}`,
    created_at: `2026-06-05T01:${String(index % 60).padStart(2, '0')}:00.000Z`,
    seq: 100 + index,
  }));
  let unboundedTailReadCount = 0;
      ...
      return {
        ...
        all(...args) {
          if (sql.includes('FROM memories')) {
            assert.deepEqual(args, ['char-a']);
            return [{ content: '主人希望画像初始化不要吞入整条长会话尾部。', category: '话题历史', importance: 0.9 }];
          }
          if (sql.includes('FROM conversations')) {
            assert.deepEqual(args, ['char-a']);
            return [{ id: 'conv-long' }];
          }
          if (sql.includes("role = 'user'") && sql.includes('ORDER BY RANDOM() LIMIT 10')) {
            assert.deepEqual(args, ['conv-long']);
            return sampledUsers;
          }
          if (sql.includes("role IN ('user','assistant')") && sql.includes('created_at >= ?')) {
            unboundedTailReadCount += unexpectedTailRows.length;
            return unexpectedTailRows;
          }
          if (sql.includes('WITH sampled_users')) {
            assert.equal(args.at(-1), 'conv-long');
  ...
  const route = requireFreshWithMocks('../src/app/api/memory-profile/route.ts', {
    ...
    '@/lib/memory-profile': {
      ...
      rollbackMemoryProfile: () => ({ character_id: 'char-a' }),
  ...
  assert.equal(payload.ok, true);
  assert.equal(unboundedTailReadCount, 0);
  assert.match(capturedSourceText, /采样用户消息 1/);
  assert.match(capturedSourceText, /采样助手回复 1/);
  assert.match(capturedSourceText, /采样用户消息 10/);
  assert.match(capturedSourceText, /采样助手回复 10/);
  assert.doesNotMatch(capturedSourceText, /未采样尾部消息/);
});
