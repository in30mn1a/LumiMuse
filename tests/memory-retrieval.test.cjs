...
test('全量注入不受历史 300 条候选上限截断（回归）', async () => {
  ...
  const insert = db.prepare(`
    ...
    VALUES (?, 'char-a', '话题历史', ?, 0.9, '[]', '[]', 'general', 0.5, 0, 'active', 0, NULL, 0, '{}', ?, ?)
  `);
  const insertAll = db.transaction(() => {
    for (let i = 0; i < 320; i += 1) {
      const ts = `2026-01-01T00:00:${String(i % 60).padStart(2, '0')}.${String(i).padStart(3, '0')}Z`;
      insert.run(`bulk-${String(i).padStart(3, '0')}`, `第 ${i} 条记忆。`, ts, ts);
    }
  });
  insertAll();
  ...
  const result = await freshRetrieval.retrieveWorkingMemoryPackage({
    characterId: 'char-a',
    queryText: '全量注入不应被条数上限截断',
    settings: baseSettings({
      limit_inject: false,
      memory_engine: {
        enabled: false,
        memory_package_token_budget: 32000,
      },
    }),
    deps: {
  ...
  assert.equal(result.mode, 'full');
  assert.equal(result.selectedMemories.length, 320);
});
...
test('memory_package_token_budget 超大值不再被钳制,预算大小由用户决定', () => {
  ...
  assert.equal(config.memory_package_token_budget, 999999);
});
